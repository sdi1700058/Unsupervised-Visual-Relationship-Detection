...
def scan(ann_dir, frames_dir, category=None):
    ...
    for f in sorted(glob.glob(os.path.join(ann_dir, "*.json"))):
        ...
        if n:
            ...
            n_frames += n
            n_trans  += max(n - 1, 0)
    return n_videos, n_frames, n_trans


def main():
    p = argparse.ArgumentParser()
    p.add_argument("--ann-dir",    default="data/vidvrd/annotations/train")
    p.add_argument("--frames-dir", default=None,
                   help="default: data/vidvrd/frames_<fps>fps/train")
    ...
    if args.frames_dir is None:
        args.frames_dir = f"data/vidvrd/frames_{args.fps}fps/train"

    if args.category:
        v, f, t = scan(args.ann_dir, args.frames_dir, args.category)
        print(f"category={args.category}  videos={v}  frames={f}  transitions={t}")
    ...
    v_all, f_all, t_all = scan(args.ann_dir, args.frames_dir)
    print(f"ALL CATEGORIES   videos={v_all}  frames={f_all}  transitions={t_all}")
    print(f"viable threshold = {args.threshold} sequential transitions")
    print()
    print(f"{'category':30s} {'videos':>6} {'transitions':>12} viable")
    print("-" * 60)
    for cat, n_vids in cats_in_videos.most_common():
        v, f, t = scan(args.ann_dir, args.frames_dir, cat)
        flag = "YES" if t >= args.threshold else "no"
        print(f"{cat:30s} {n_vids:>6d} {t:>12d}  {flag}")
